...
namespace ESS.Framework.Common.Scheduling
{
    ...
    public class Worker
    {
        ...
        /// <summary>
        ///     Returns the action name of the current worker.
        /// </summary>
        ...
        public Worker Start()
        {
            if (_currentState != null && !_currentState.StopRequested)
            {
                return this;
            }

            var thread = new Thread(Loop) { Name = string.Format("{0}.Worker", _actionName), IsBackground = true };
            var state = new WorkerState();

            thread.Start(state);

            _currentState = state;
            _logger.DebugFormat("Worker started, actionName:{0}, id:{1}, managedThreadId:{2}, nativeThreadId:{3}", _actionName, state.Id,
                thread.ManagedThreadId, GetNativeThreadId(thread));
        ...
        {
            public readonly string Id = ObjectId.GetNextGuid().ToString();
