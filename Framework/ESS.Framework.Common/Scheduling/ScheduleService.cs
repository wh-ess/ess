        ...
        public int ScheduleTask(string actionName, Action action, int dueTime, int period)
        {
            ...
            var timer = new Timer(obj =>
            {
                ...
                if (_taskDict.TryGetValue(currentTaskId, out currentTask))
                {
                    ...
                    {
                        _logger.Error(
                            string.Format("Task has exception, actionName:{0}, dueTime:{1}, period:{2}", currentTask.ActionName, currentTask.DueTime,
                                currentTask.Period), ex);
                    }
                    finally
                    {
                        if (!currentTask.Stoped)
            ...
            if (
                !_taskDict.TryAdd(newTaskId,
                    new TimerBasedTask { ActionName = actionName, Action = action, Timer = timer, DueTime = dueTime, Period = period, Stoped = false }))
            {
