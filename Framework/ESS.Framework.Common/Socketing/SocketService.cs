    ...
    public class SocketService
    {
        ...
        private void ReceiveCallback(IAsyncResult asyncResult)
        {
            ...
            if (bytesRead <= 0)
            {
                ...
                _logger.DebugFormat(
                    "Source socket EndReceive completed, but no bytes were read, close the source socket, source socket address:{0}, receiveSize:{1}",
                    sourceSocketInfo.SocketRemotingEndpointAddress, receiveState.ReceiveSize);
                return;
            }
            ...
            if (receiveState.MessageSize == null)
            {
                if (bytesRead < SocketUtils.MessageHeaderLength)
                {
                    ...
                    if (remainSize > 0)
                    {
                        _logger.DebugFormat("Received part of message header, receivedSize:{0}, remainSize:{1}, bytesRead:{2}", receivedData.Count,
            ...
            if (receivedData.Count < receiveState.MessageSize.Value)
            {
                ...
                _logger.DebugFormat("Receive part of Message, receivedSize:{0}, remainSize:{1}, messageSize:{2}", receivedData.Count, remainSize,
                    receiveState.MessageSize.Value);
            ...
            else
            {
                ...
                {
                    _logger.Error(
                        string.Format("Exception raised when calling MessageReceivedCallback, source socket address:{0}",
                            sourceSocketInfo.SocketRemotingEndpointAddress), ex);
                }
