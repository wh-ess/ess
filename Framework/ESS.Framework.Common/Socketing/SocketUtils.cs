...
namespace ESS.Framework.Common.Socketing
{
    public class SocketUtils
    {
        ...
        public static bool IsSocketDisconnectedException(SocketException socketException)
        {
            ...
            return errorCode == SocketError.NetworkDown || errorCode == SocketError.NetworkReset || errorCode == SocketError.NetworkUnreachable
                   || errorCode == SocketError.ConnectionAborted || errorCode == SocketError.ConnectionReset || errorCode == SocketError.HostNotFound
                   || errorCode == SocketError.HostUnreachable || errorCode == SocketError.NotConnected || errorCode == SocketError.Shutdown
                   || errorCode == SocketError.Disconnecting || errorCode == SocketError.AddressNotAvailable || errorCode == SocketError.TimedOut;
        }
