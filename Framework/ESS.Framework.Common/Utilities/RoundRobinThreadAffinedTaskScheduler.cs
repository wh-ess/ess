...
{
    /// <summary>
    ///     An implementation of <see cref="TaskScheduler" /> which creates an underlying thread pool and set processor
    ///     affinity to each thread.
    /// </summary>
    public sealed class RoundRobinThreadAffinedTaskScheduler : TaskScheduler, IDisposable
    {
        ...
        public RoundRobinThreadAffinedTaskScheduler(int numberOfThreads)
        {
            ...
            var processorIndexes = Enumerable.Range(0, Environment.ProcessorCount)
                .ToArray();
            ...
            foreach (var processorIndex in processorIndexes)
            {
                if (processorIndex >= Environment.ProcessorCount || processorIndex < 0)
                {
                    throw new ArgumentOutOfRangeException("processorIndexes",
                        string.Format("processor index {0} was supperior to the total number of processors in the system", processorIndex));
                }
            }
        ...
        private static ProcessThread CurrentProcessThread
        {
            get
            {
                ...
                foreach (ProcessThread processThread in Process.GetCurrentProcess()
                    .Threads)
                {
                ...
                throw new InvalidOperationException(string.Format("Could not retrieve native thread with ID: {0}, current managed thread ID was {1}",
        ...
        /// <summary>
        ///     Generates an enumerable of <see cref="T:System.Threading.Tasks.Task" /> instances currently queued to the scheduler
